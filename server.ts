...
async function startServer() {
  ...
  app.put('/api/organization/team/:userId/role', requireAuth, requireRole(['Owner', 'Admin']), validateBody(teamRoleSchema), async (req: AuthenticatedRequest, res) => {
    try {
      ...
      await db.update(usersTable)
        .set({ role })
        .where(and(eq(usersTable.id, parseInt(userId)), eq(usersTable.tenantId, tenantId)));
  ...
  app.delete('/api/organization/team/:userId', requireAuth, requireRole(['Owner', 'Admin']), async (req: AuthenticatedRequest, res) => {
    try {
      ...
      await db.delete(usersTable).where(and(eq(usersTable.id, parseInt(userId)), eq(usersTable.tenantId, tenantId)));
  ...
  app.post('/api/billing/webhook', async (req: express.Request & { rawBody?: Buffer }, res: express.Response) => {
    ...
    try {
      ...
      if (event.type === 'checkout.session.completed') {
        ...
        if (billingId && webhookTenantId) {
          await db.update(billingTable)
            .set({ status: 'Paid' })
            .where(and(eq(billingTable.id, billingId), eq(billingTable.tenantId, webhookTenantId)));
          console.log(`[Stripe Webhook] Verified payment event for invoice: ${billingId}`);
        } else if (billingId) {
          console.error(`[Stripe Webhook] checkout.session.completed missing tenantId in metadata for invoice: ${billingId} — refusing to mark paid without tenant scope`);
        }
      }
