...
describe('CycloneDX validation and normalization', () => {
  ...
  it('maps every manifest ecosystem inspectTree() discovers to its real OSV.dev ecosystem name, never defaulting to npm', () => {
    const normalized = normalizeCycloneDx({
      bomFormat: 'CycloneDX',
      components: [
        { name: 'github.com/pkg/errors', version: '0.9.1', purl: 'pkg:golang/github.com/pkg/errors@0.9.1' },
        { name: 'serde', version: '1.0.0', purl: 'pkg:cargo/serde@1.0.0' },
        { name: 'org.apache.commons:commons-lang3', version: '3.12.0', purl: 'pkg:maven/org.apache.commons/commons-lang3@3.12.0' },
        { name: 'Newtonsoft.Json', version: '13.0.1', purl: 'pkg:nuget/Newtonsoft.Json@13.0.1' },
        { name: 'rails', version: '7.0.0', purl: 'pkg:gem/rails@7.0.0' },
        { name: 'monolog/monolog', version: '2.0.0', purl: 'pkg:composer/monolog/monolog@2.0.0' },
        // No purl at all — must stay unresolved rather than assumed to be npm.
        { name: 'mystery-package', version: '1.0.0' },
      ],
    });
    const byName = Object.fromEntries(normalized.map(c => [c.name, c.ecosystem]));
    expect(byName['github.com/pkg/errors']).toBe('Go');
    expect(byName['serde']).toBe('crates.io');
    expect(byName['org.apache.commons:commons-lang3']).toBe('Maven');
    expect(byName['Newtonsoft.Json']).toBe('NuGet');
    expect(byName['rails']).toBe('RubyGems');
    expect(byName['monolog/monolog']).toBe('Packagist');
    expect(byName['mystery-package']).toBeUndefined();
  });
});
